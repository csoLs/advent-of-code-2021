...
const calculatePayload = (pkg: IPacket): number => {
  if(pkg.typeId === 0) return pkg.subPackets.reduce((acc, v) => acc + calculatePayload(v), 0)
  if(pkg.typeId === 1) return pkg.subPackets.reduce((acc, v) => acc * calculatePayload(v), 1)
  if(pkg.typeId === 2) return pkg.subPackets.reduce((acc, v) => Math.min(acc, calculatePayload(v)), Number.MAX_SAFE_INTEGER)
  if(pkg.typeId === 3) return pkg.subPackets.reduce((acc, v) => Math.max(acc, calculatePayload(v)), 0)
  if(pkg.typeId === 4) return pkg.payload || 0
  if(pkg.typeId === 5) return calculatePayload(pkg.subPackets[0]) > calculatePayload(pkg.subPackets[1]) ? 1 : 0 
  if(pkg.typeId === 6) return calculatePayload(pkg.subPackets[0]) < calculatePayload(pkg.subPackets[1]) ? 1 : 0 
  if(pkg.typeId === 7) return calculatePayload(pkg.subPackets[0]) === calculatePayload(pkg.subPackets[1]) ? 1 : 0 
...
const fn = (input: string) => {
  ...
  return calculatePayload(decodedPacket.thisPackage) 
}
