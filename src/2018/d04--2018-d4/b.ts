...
const calculateMostCommon = (ranges: number[][]) => {
  ...
  return {
    count: highestSleepMinute,
    mostAsleepOn: minutes.findIndex(v => highestSleepMinute === v) +1
  }
}
...
const fn = (input: string[]) => {
  ...
  const relevantGuard =
    Object.values(guardSleepTime)
      .map(row => ({
        guardId: row.guardId,
        ...calculateMostCommon(row.sleepRanges)
      })).sort((a,b) => b.count-a.count)[0]

      console.log(relevantGuard)

  return relevantGuard.guardId*relevantGuard.mostAsleepOn
}
